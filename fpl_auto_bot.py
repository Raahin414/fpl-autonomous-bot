...
- Fully uses secrets from GitHub Actions.
"""
...
# -------------------------
# Authenticated Session (fixed)
# -------------------------
def fpl_login() -> requests.Session:
    print("[LOGIN] Starting secure login…")
    ...
    if not csrf:
        print("[LOGIN] Warning: CSRF token missing, continuing anyway.")
    ...
    res = session.post(LOGIN_URL, data=payload, headers=headers, allow_redirects=True, timeout=30)
    if res.status_code not in (200, 302):
        raise RuntimeError(f"[LOGIN] Failed with status {res.status_code}")

    if not session.cookies.get_dict():
        print("[LOGIN] Warning: cookies empty after login, may need manual intervention.")
    else:
        print("[LOGIN] Logged in, cookies present.")
...
def post_transfers(session: requests.Session, payload: dict, note: str):
    ...
    print(f"[POST] Transfers ({note}) → {r.status_code}")
...
def post_picks(session: requests.Session, picks_payload: dict):
    ...
    print(f"[POST] Picks → {r.status_code}")
    ...
    if 0 < hrs <= 24:
        ...
        if len(outs) == len(ins) and len(ins) > 0:
            for o, i in zip(outs, ins):
                transfers.append({"element_out": int(o), "element_in": int(i), "purchase_price": 0, "selling_price": 0})

...
- Supports PL_SESSION cookie or Playwright login as fallback.
...
# Optional Playwright import for login
try:
...
PL_SESSION = os.getenv("PL_SESSION")  # Optional cookie fallback
...
def get_session_from_cookie(cookie: str) -> requests.Session:
    s = requests.Session()
    s.headers.update({
    ...
    s.cookies.set("pl_session", cookie, domain=".fantasy.premierleague.com")
    return s

def get_fpl_session(email: str, password: str) -> requests.Session:
    if PL_SESSION:
        print("[LOGIN] Using PL_SESSION cookie...")
        s = get_session_from_cookie(PL_SESSION)
        try:
            r = s.get(MY_TEAM_URL, timeout=15)
            if r.status_code == 200:
                print("[LOGIN] PL_SESSION valid.")
                return s
            else:
                print("[LOGIN] PL_SESSION invalid, falling back to Playwright...")
        except Exception:
            print("[LOGIN] PL_SESSION request failed, falling back to Playwright...")

    if not PLAYWRIGHT_AVAILABLE:
        raise RuntimeError("[LOGIN] Playwright not installed and PL_SESSION invalid.")
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            browser.close()
            raise RuntimeError("Playwright login failed: timeout waiting for login form")
        ...
        try:
            page.wait_for_selector("a[href='/my-team']", timeout=45000)
        except Exception:
            browser.close()
            raise RuntimeError("Playwright login failed: login did not complete")
        ...
        for c in cookies:
            if c['name'] == "pl_session":
                s.cookies.set("pl_session", c['value'], domain=".fantasy.premierleague.com")
                print("[LOGIN] Obtained PL_SESSION from Playwright.")
                return s
        raise RuntimeError("Playwright login succeeded but PL_SESSION cookie not found")
...
def weekly_routine():
    ...
    session = get_fpl_session(FPL_EMAIL, FPL_PASSWORD)
    ...
    if 0 < hrs <= 24:
        ...
        if len(outs) == len(ins) and len(ins) > 0:
            for o, i in zip(outs, ins):
                transfers.append({"element_out": o, "element_in": i, "wildcard": False})

        xi, cap, vice = pick_xi(scored, desired)

        if transfers:
            transfer_payload = {"entry": TEAM_ID, "event": gw, "transfers": transfers, "chip": None}
            post_transfers(session, transfer_payload, note="Free transfers within 24h")
        picks_payload = {
            "picks": [
                {"element": int(pid), "position": idx+1, "is_captain": int(pid)==int(cap), "is_vice_captain": int(pid)==int(vice)}
                for idx, pid in enumerate(xi)
        ...
        post_picks(session, picks_payload)
        print("[GW] Picks & free transfers applied.")
        return

    print("[GW] Outside action window. Exiting.")
